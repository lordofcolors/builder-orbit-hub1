...
export default function SessionEnd() {
  ...
  const fullTranscript: TranscriptEntry[] = [
    ...
    },
    {
      id: "5",
      type: "agent",
      content: "Excellent question! Hold the leash with a relaxed grip about 18 inches from the dog's collar. This gives them freedom while maintaining control.",
    ...
    },
    {
      id: "7",
      type: "agent",
      content: "When your dog pulls, stop walking immediately. Wait for them to look back at you, then reward with praise and continue. Consistency is key!",
    ...
    <div className="min-h-screen bg-app-bg p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-app-text mb-2">Session Complete</h1>
          <p className="text-app-muted text-lg">Master Dog Walking - Session Duration: 15 minutes</p>
        </div>

        {/* Action Buttons */}
        <div className="flex justify-center gap-4 mb-8">
          <Button 
            className="bg-app-primary text-app-primary-dark hover:bg-app-primary/90 flex items-center gap-2"
          >
                  ...
                  <div className="flex-1">
                    <div className="flex items-start gap-2">
                      <span className={`font-semibold text-sm ${
                        entry.type === 'agent' ? 'text-app-agent' : 'text-app-user'
                      }`}>
                        {entry.type === 'agent' ? 'Assistant:' : 'You:'}
                      </span>
                    </div>
                    <p className={`text-sm mt-1 ${
                      entry.type === 'agent' ? 'text-app-agent' : 'text-app-text'
                    }`}>
                      {entry.content}
                    </p>
                  </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-app-bg border border-app-border rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-app-primary mb-1">15:42</div>
