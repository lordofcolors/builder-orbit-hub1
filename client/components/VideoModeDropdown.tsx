...
export default function VideoModeDropdown({ currentMode, onModeChange }: VideoModeDropdownProps) {
            ...
            {modes.map((mode) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 text-sm rounded-md hover:bg-app-border/20 transition-colors ${
                  currentMode === mode.value ? 'bg-app-border/20 text-app-primary' : 'text-app-text'
                }`}
              >
                {mode.icon}
                <div className="text-left">
                  <div className="font-medium">{mode.label}</div>
                  <div className="text-xs text-app-muted">{mode.description}</div>
                </div>
              </button>
            ))}
          </div>
